...
- 日期：2026-09-10
- 状态：已实现（2026-09-10，113 个单测全绿 + debug/release 构建通过；真机 UI 手工验证待做）
...
9. 删除笔记：历史一并清空。
10. 新建空笔记保存后再编辑：历史正常。
11. 已有历史的笔记导入较新备份后，打开历史页确认"当前版本"标记与内容关系（已知会失真，见 §14）。
...
| GC 与并发写入的 TOCTOU 窗口（读引用集后、删文件前出现新引用） | 已接受：应用为单用户顺序操作，窗口毫秒级，且与既有"删除笔记触发 GC"同类；事务提交后才在 IO 线程执行 GC |
| 备份导入直接覆写笔记（不走 `saveNote`），已有历史的笔记导入较新备份后"当前版本"标记可能失真 | 已接受（非目标：不改备份行为）：导入内容在下次保存产生新快照前可能不出现在历史中；真机清单第 11 项人工确认 |
