...
| 图片加载 | Coil 2.7.0 | 轻量、协程 + Compose 原生 |
...
> v1 修订（2026-09-09）：编辑态暂为纯文本输入 + 预览模式切换，图片块「可点击/可删除」交互缓做；拍照插入缓做，仅保留相册 Photo Picker。详见实现计划修订记录。
...
- **拍照**：系统相机 App 拍照，落回私有目录。（v1 缓做，见 §6 修订注）
